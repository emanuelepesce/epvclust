...
{ 
	...
	if (toPrint == 1){
		printf("============ Distance matrix between genes ============\n");
		printf("   Gene:");
		for(i=0; i<nrows-1; i++) 
			printf("%6d", i);
		printf("\n");
...
method	char
method to use for computing hierarchical clustering
 ...
 * Purpose
 * =======
 * Same of hclustering() with mask passed. Calculating mask outside the
 * function could emprove the performances if the hclusteringM is in a loop;
 *
 * Mask has the same size of data. Elements are in range {0,1}. 0 if element is missing, 1 if it's ok.
 */
{
    int i,j;
    double* weight = malloc(ncols*sizeof(double));
